...
export class ListDetailsPage implements OnInit {
  ...
  async addTodoModal() {
      ...
      this.uiService.presentToast("Vous n'êtes pas autorisé à effectuer cette opération", "danger", 3000)
    }  
  }
  ...
  onCallNumber(number: string){
    if(Capacitor.isNative){
      this.callNumber.callNumber(number, true)
        .catch(err => {
          this.uiService.presentToast("Échec de l'appel : "+err, "danger", 3000)
        });
      }    
  }
}
